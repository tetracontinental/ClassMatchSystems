...
import { SoccerMatch, SoccerStatistics } from '@/app/types/soccer'

interface SoccerRanking {
  id: string
  className: string
  gender: 'men' | 'women'
  rank: number | null
...
export default function SoccerResults() {
  const [menMatches, setMenMatches] = useState<SoccerMatch[]>([])
  const [womenMatches, setWomenMatches] = useState<SoccerMatch[]>([])
  const [menRankings, setMenRankings] = useState<SoccerRanking[]>([])
  const [womenRankings, setWomenRankings] = useState<SoccerRanking[]>([])
  const [statistics, setStatistics] = useState<SoccerStatistics | null>(null)
  ...
  const fetchAllData = async () => {
    try {
      ...
      // 男子試合データ取得
      const menResponse = await fetch('/api/soccer/matches?gender=men')
      if (menResponse.ok) {
      ...
      // 女子試合データ取得
      const womenResponse = await fetch('/api/soccer/matches?gender=women')
      if (womenResponse.ok) {
      ...
      // 男子順位データ取得
      const menRankingsResponse = await fetch('/api/soccer/rankings?gender=men')
      ...
      // 女子順位データ取得
      const womenRankingsResponse = await fetch('/api/soccer/rankings?gender=women')
      ...
      // 統計データ取得
      const statsResponse = await fetch('/api/soccer/statistics')
      if (statsResponse.ok) {
  ...
  const getMatchStatusDisplay = (match: SoccerMatch) => {
    switch (match.status) {
      case 'waiting':
        return {
          text: '開始前',
          color: 'bg-gray-100 text-gray-700',
          time: match.scheduledTime ? `予定: ${new Date(match.scheduledTime).toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })}` : ''
        }
      case 'in_progress':
        return {
          text: 'LIVE',
  ...
  const calculateTotalScore = (match: SoccerMatch, team: 'team1' | 'team2') => {
        ...
        <div className="bg-white rounded-lg shadow-md border p-6">
          ...
          <div className="space-y-8">
            {[1, 2, 3, 4, 5].map(round => {
              ...
              return (
                <div key={round} className="border-l-4 border-green-500 pl-4">
                    ...
                    {roundMatches.map(match => {
                      ...
                      return (
                        <div key={match.id} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                          ...
                          {/* PK戦の結果表示 */}
                          {match.pkScores && (
                            <div className="mt-2 text-center p-2 bg-yellow-100 rounded">
                              <span className="text-sm font-medium text-yellow-800">
                                PK戦: {match.pkScores.team1} - {match.pkScores.team2}
                              </span>
                            </div>
                          )}
                    ...
                    <div className="text-center">
                      ...
                      {match.pkScores && (
                        <div className="text-xs text-yellow-600">
                          PK: {match.pkScores.team1} - {match.pkScores.team2}
                        </div>
                      )}
          ...
          <div className="bg-white rounded-lg shadow-md border p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div className="text-center p-4 bg-green-50 rounded-lg">
                <div className="text-3xl font-bold text-green-600">{statistics.men.totalMatches + statistics.women.totalMatches}</div>
                <div className="text-sm text-gray-600">総試合数</div>
              </div>
              <div className="text-center p-4 bg-blue-50 rounded-lg">
                <div className="text-3xl font-bold text-blue-600">{statistics.men.completedMatches + statistics.women.completedMatches}</div>
                <div className="text-sm text-gray-600">完了試合数</div>
              </div>
              <div className="text-center p-4 bg-orange-50 rounded-lg">
                <div className="text-3xl font-bold text-orange-600">
                  {Math.max(
                    statistics.men.highestScoringMatch?.totalGoals || 0,
                    statistics.women.highestScoringMatch?.totalGoals || 0
                  )}
                </div>
                <div className="text-sm text-gray-600">最高得点試合</div>
                <div className="text-xs text-gray-500">
                  {(statistics.men.highestScoringMatch?.totalGoals || 0) >= (statistics.women.highestScoringMatch?.totalGoals || 0)
                    ? statistics.men.highestScoringMatch?.teams
                    : statistics.women.highestScoringMatch?.teams}
                </div>
              </div>
              <div className="text-center p-4 bg-purple-50 rounded-lg">
                <div className="text-3xl font-bold text-purple-600">{statistics.men.pkMatches + statistics.women.pkMatches}</div>
                <div className="text-sm text-gray-600">PK戦決着</div>
              </div>
            </div>
          </div>
        )}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      <div className={`p-4 rounded-lg border-2 ${
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between">
                            <span>前半</span>
                            <span className="font-bold">{match.scores?.team1?.firstHalf || 0}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>後半</span>
                            <span className="font-bold">{match.scores?.team1?.secondHalf || 0}</span>
                          </div>
                          {match.pkScores && (
                            <div className="flex justify-between">
                              <span>PK戦</span>
                              <span className="font-bold text-yellow-600">{match.pkScores.team1}</span>
                            </div>
                          )}
                      ...
                      <div className={`p-4 rounded-lg border-2 ${
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between">
                            <span>前半</span>
                            <span className="font-bold">{match.scores?.team2?.firstHalf || 0}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>後半</span>
                            <span className="font-bold">{match.scores?.team2?.secondHalf || 0}</span>
                          </div>
                          {match.pkScores && (
                            <div className="flex justify-between">
                              <span>PK戦</span>
                              <span className="font-bold text-yellow-600">{match.pkScores.team2}</span>
                            </div>
                          )}
                          <div className="border-t pt-2 flex justify-between text-lg font-bold">
                            <span>合計</span>
