...
import { SoftballMatch, SoftballStatistics, SoftballRanking } from '@/app/types/softball'

export default function SoftballResults() {
  const [matches, setMatches] = useState<SoftballMatch[]>([])
  const [statistics, setStatistics] = useState<SoftballStatistics | null>(null)
  const [rankings, setRankings] = useState<SoftballRanking[]>([])
  ...
  const fetchAllData = async () => {
    try {
      ...
      // 試合データ取得
      const matchesResponse = await fetch('/api/softball/matches')
      ...
      // 統計データ取得
      const statsResponse = await fetch('/api/softball/statistics')
      if (statsResponse.ok) {
      ...
      // 順位データ取得
      const rankingsResponse = await fetch('/api/softball/rankings')
  ...
  const getMatchStatusDisplay = (match: SoftballMatch) => {
    switch (match.status) {
      case 'waiting':
        return {
          text: '開始前',
  ...
  const calculateTotalScore = (innings: number[]) => {
    return innings.reduce((total, score) => total + score, 0)
  }
  ...
  const hasExtensionInnings = (match: SoftballMatch) => {
    const team1Innings = match.scores.team1.innings
    const team2Innings = match.scores.team2.innings
    
    // 5回目以降（インデックス4以降）にスコアがあるかチェック
    for (let i = 4; i < Math.max(team1Innings.length, team2Innings.length); i++) {
      if ((team1Innings[i] || 0) > 0 || (team2Innings[i] || 0) > 0) {
        return true
      }
    }
    return false
                ...
                <div key={round} className="border-l-4 border-blue-500 pl-4">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {roundMatches.map(match => {
                      ...
                      const team1Total = calculateTotalScore(match.scores.team1.innings)
                      const team2Total = calculateTotalScore(match.scores.team2.innings)
                      const isExtension = hasExtensionInnings(match)
                      ...
                      return (
                        <div key={match.id} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                          ...
                          <div className="mt-2 flex flex-wrap gap-1">
                            ...
                            {match.isJankenNeeded && match.jankenWinner && (
                  ...
                  <div key={match.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex gap-1">
                        {isExtension && (
                          <span className="text-xs bg-orange-100 text-orange-800 px-1 py-0.5 rounded">延長</span>
                        )}
                        {match.isJankenNeeded && (
                          <span className="text-xs bg-purple-100 text-purple-800 px-1 py-0.5 rounded">じゃんけん</span>
                        )}
                      </div>
                    </div>
                    <div className="text-center">
          ...
          <div className="bg-white rounded-lg shadow-md border p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              ...
              <div className="text-center p-4 bg-purple-50 rounded-lg">
                <div className="text-3xl font-bold text-purple-600">{statistics.averageScore}</div>
                <div className="text-sm text-gray-600">平均得点</div>
              </div>
              <div className="text-center p-4 bg-yellow-50 rounded-lg">
                <div className="text-3xl font-bold text-yellow-600">{statistics.extensionMatches}</div>
                <div className="text-sm text-gray-600">延長戦数</div>
              </div>
              <div className="text-center p-4 bg-pink-50 rounded-lg">
                <div className="text-3xl font-bold text-pink-600">{statistics.jankenMatches}</div>
                <div className="text-sm text-gray-600">じゃんけん決着</div>
              </div>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex gap-2">
                          {isExtension && (
                            <span className="px-2 py-1 bg-orange-100 text-orange-800 rounded text-sm">延長戦</span>
                          )}
                          {match.isJankenNeeded && match.jankenWinner && (
                            <span className="px-2 py-1 bg-purple-100 text-purple-800 rounded text-sm">じゃんけん決着</span>
                          )}
                        </div>
                      </div>
                      <div className="text-sm text-gray-500">
                      ...
                      <div className={`p-4 rounded-lg border-2 ${
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-7 gap-1 text-sm">
                            {['1回', '2回', '3回', '4回', '延1', '延2', '延3'].map((inning, index) => (
                              <div key={inning} className="text-center">
                                <div className="text-xs text-gray-500 mb-1">{inning}</div>
                                <div className="font-bold bg-white rounded px-1 py-0.5">
                                  {match.scores.team1.innings[index] || 0}
                                </div>
                              </div>
                            ))}
                          </div>
                          ...
                          <div className="grid grid-cols-7 gap-1 text-sm">
                            {['1回', '2回', '3回', '4回', '延1', '延2', '延3'].map((inning, index) => (
                              <div key={inning} className="text-center">
                                <div className="text-xs text-gray-500 mb-1">{inning}</div>
                                <div className="font-bold bg-white rounded px-1 py-0.5">
                                  {match.scores.team2.innings[index] || 0}
                                </div>
                              </div>
                            ))}
                          </div>
                      ...
                      <div className="mt-4 p-3 bg-purple-50 border border-purple-200 rounded-lg">
                        <div className="text-center">
                          <span className="text-purple-800 font-medium">
                            ✂️ じゃんけんで決着: {match.jankenWinner} の勝利
                          </span>
                        </div>
                      </div>
                    )}
                        ...
                        return (
                          <div key={match.id} className="border border-gray-200 rounded p-1 lg:p-2 text-xs lg:text-sm">
                            ...
                            <div className={`flex justify-between items-center py-0.5 lg:py-1 px-1 lg:px-2 rounded ${
                              ...
                              <span className="font-bold ml-1 lg:ml-2 text-xs lg:text-sm">{team1Total}</span>
                            </div>
                            <div className={`flex justify-between items-center py-0.5 lg:py-1 px-1 lg:px-2 rounded mt-0.5 lg:mt-1 ${
                              ...
                              <span className="font-bold ml-1 lg:ml-2 text-xs lg:text-sm">{team2Total}</span>
                            </div>
